    ...
    try {
      if (message.mentions.channels.size === 0) {
        ...
        moveerMessage.sendMessage(
          message,
          'You need to specify what channel to use, by tagging it with #channelname',
        );
        return;
      }
      ...
      moveerMessage.logger(
        message,
        'Added moveeradmin channel with name: ' + message.mentions.channels.first().name,
      );
      moveerMessage.sendMessage(
        message,
      ...
      throw {
        logmessage: 'Error',
        sendMessage:
          "Moveer cannot communicate with it's database. Since this is a admin command please create a textchannel named moveeradmin and use that until my developer fixes this! He has been alerted but please poke him inside the support server! https://discord.gg/dTdH3gD",
      };
    }
  } catch (err) {
