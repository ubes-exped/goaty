...
async function move(args, message, rabbitMqChannel) {
  try {
    ...
    if (
      message.channel.name.toLowerCase() !== 'av-numbers' &&
      message.channel.name.toLowerCase() !== 'av-verified'
    )
      return;
    ...
    if (parseInt(toVoiceChannelName) <= 500) {
      ...
      toVoiceChannelName === 'l' ||
      toVoiceChannelName === 'lobby' ||
      toVoiceChannelName === 'q' ||
      toVoiceChannelName === 'queue'
    ) {
      toVoiceChannelName = 'Lobby';
    } else {
