...
async function move(args, message, rabbitMqChannel) {
  try {
    ...
    helper.checkUserAmountInChannel(
      message,
      fromVoiceChannel.members.size,
      amountInEachChannel,
    ...
    for (let i = 0; i < userIdsLength; i++) {
      ...
      helper.checkIfCatergyHasRoomsAvailable(
        message,
        voiceChannelCounter,
      ...
      helper.checkUserAmountInChannel(
        message,
        userIdsToMove.length,
        amountInEachChannel,
        fromVoiceChannelName,
      );

      const randomUsersTomove = await getRandomUsers(userIdsToMove, amountInEachChannel);
      await helper.checkForMovePerms(
        message,
        userIdsToMove,
        voiceChannelsInCategory[voiceChannelCounter],
      );
      await helper.checkForConnectPerms(
        message,
        userIdsToMove,
        voiceChannelsInCategory[voiceChannelCounter],
      );
