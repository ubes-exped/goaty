...
function checkIfVoiceChannelExist(message, voiceChannel, channelName) {
  ...
  if (voiceChannel == null) {
    throw {
      ...
      sendMessage:
        command === 'move'
          ? moveerMessage.NO_VOICE_CHANNEL_NAMED_X +
            'the name/id: "' +
            channelName +
            '" <@' +
            message.author.id +
...
function checkIfUsersInsideVoiceChannel(message, fromVoiceChannelName, fromVoiceChannel) {
  ...
  if (fromVoiceChannel.members.size < 1) {
    throw {
      ...
      sendMessage:
        moveerMessage.NO_USERS_INSIDE_ROOM +
        ':  ' +
...
async function checkIfTextChannelIsMoveerAdmin(message) {
  if (message.channel.name.toLowerCase() !== 'moveeradmin') {
    ...
    if (
      ...
      searchForGuild.rows[0].adminChannelId === message.channel.id
    ) {
...
function getCategoryByName(message, categoryName) {
  ...
  if (category == null) {
    throw {
      ...
      sendMessage:
        'No category found with the name: ' + categoryName + ' <@' + message.author.id + '>',
    };
  }
  return category;
}
...
function checkIfGuildHasTwoMoveerChannels(message) {
  if (
    message.guild.channels.find(channel => channel.name === 'Moveer') !== null &&
    message.guild.channels.find(channel => channel.name === 'moveer') !== null
  ) {
    throw {
  ...
  for (let i = 0; i < messageMentions.length; i++) {
    if (message.guild.members.get(messageMentions[i].id).voiceChannelID == null) {
      ...
      moveerMessage.sendMessage(
        message,
        messageMentions[i] + ' ' + moveerMessage.USER_MENTION_NOT_IN_ANY_CHANNEL,
      );
    }
  }
...
function checkIfUsersAlreadyInChannel(message, messageMentions, toVoiceChannelId) {
  for (let i = 0; i < messageMentions.length; i++) {
    if (message.guild.members.get(messageMentions[i].id).voiceChannelID === toVoiceChannelId) {
      ...
      moveerMessage.sendMessage(
        message,
        messageMentions[i].username + ' ' + moveerMessage.USER_ALREADY_IN_CHANNEL,
      );
    }
  }
  ...
  for (let i = 0; i < users.length; i++) {
    ...
    if (await !userVoiceChannel.memberPermissions(message.guild.me).has('CONNECT')) {
      throw {
        ...
        sendMessage:
          moveerMessage.MOVEER_MISSING_CONNECT_PERMISSION +
          ' "' +
  ...
  for (let i = 0; i < users.length; i++) {
    ...
    if (await !userVoiceChannel.memberPermissions(message.guild.me).has('MOVE_MEMBERS')) {
      throw {
        ...
        sendMessage:
          moveerMessage.MOVEER_MISSING_MOVE_PERMISSION +
          ' "' +
          userVoiceChannel.name +
          '" <@' +
          message.author.id +
...
function checkIfChannelIsTextChannel(message, channel) {
  if (channel.type === 'text') {
    throw {
      ...
      sendMessage:
        channel.name +
        moveerMessage.USER_MOVED_WITH_TEXT_CHANNEL +
        ' <@' +
        message.author.id +
        '> \n',
    };
...
function checkIfCatergyHasRoomsAvailable(
  message,
  voiceChannelCounter,
...
function checkCountOfChannelsFromCategory(message, checkCountOfChannelsFromCategory, categoryName) {
  if (checkCountOfChannelsFromCategory.length === 0) {
    throw {
      ...
      sendMessage:
        'No voicechannels exists or no empty voicechannels in the category: ' + categoryName,
    };
  }
}
...
async function successfullmove(usersMoved) {
  ...
  try {
    ...
    await client.query(
      'UPDATE moves SET successCount = successCount + ' + usersMoved + ' WHERE id = 1',
    );
    await client.end();
  } catch (err) {
    console.log(err);
...
async function getMoveerAdminChannelFromDB(message, guildId) {
  try {
    ...
    await client.connect();
    const searchForGuild = await client.query(
      'SELECT * FROM "guilds" WHERE "guildId" = \'' + message.guild.id + "'",
    );
    await client.end();
